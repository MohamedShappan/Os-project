...
void env_free(struct Env *e)
{
	...
	for(int i=0;i<100;i++){
		if(!semaphores[i].empty){
			free_semaphore_object(get_semaphore_object_ID(semaphores[i].ownerID,semaphores[i].name));
		}
	}
...
void * create_user_page_WS(struct Env * e, unsigned int numOfElements)
{
	...
	struct WorkingSetElement* ptr=kmalloc(nbytes);
	if(ptr==NULL)
		panic("NOT ENOUGH KERNAL SPACE");
	return ptr;
